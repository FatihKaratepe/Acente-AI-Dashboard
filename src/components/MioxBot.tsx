...
export const MioxBot = forwardRef<MioxBotHandle>((_, ref) => {
  ...
  const streamResponse = async (responseText: string) => {
    ...
    setMessages((prev) => [...prev, { id: messageId, text: responseText, sender: 'bot', isStreaming: true }]);
  };
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4 font-sans pointer-events-none">
      ...
      >
        <Card className="w-87.5 shadow-2xl border-primary/20 bg-background/95 backdrop-blur-md supports-backdrop-filter:bg-background/60 py-0 gap-2">
          ...
          <CardContent className="h-100 overflow-y-auto p-4 flex flex-col gap-4">
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={cn(
              ...
              >
                {msg.isStreaming ? (
                  <StreamingMessage text={msg.text} onComplete={() => handleStreamingComplete(msg.id)} />
                ) : (
                  msg.text
                )}
                {msg.isStreaming && (
          ...
          <CardFooter className="p-3 pt-0">
            ...
            >
              <Textarea
                ...
                className="flex-1 min-h-10 max-h-30 rounded-2xl bg-muted/50 border-transparent focus-visible:bg-background focus-visible:ring-1 focus-visible:ring-primary/20 pl-4 resize-none py-2"
              />
              <Button
                type="submit"
                size="icon"
